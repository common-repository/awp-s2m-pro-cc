...
echo '<th class="-affiliate"><i class="fa fa-share-alt"></i> Affiliate</td>' . "\n";
...
foreach ( $_coupons->coupons as $_coupon ) {
	...
	echo '<td class="-affiliate">';
	?>
<input type="text" name="user_name" value="<?php echo esc_attr( $_coupon['affiliate'] ? $_coupon['affiliate'] : '' ); ?>" class="affwp-user-search" data-affwp-status="active" autocomplete="off" />
	<?php
	echo '</td>';
...
echo '<li style="list-style:none;margin-top:1em;"><strong><i class="fa fa-share-alt"></i> Affiliate</strong>&nbsp;&nbsp;&nbsp; <em>s2Member checks this field to assign this coupon to the affiliate registered on AffiliateWP. You need to input the Affiliate user login (username).</em></li>' . "\n";
echo '</ul>' . "\n";
...
echo '<ul>' . "\n";
echo '<li><code>SILENT-PHP|URL goes here</code> = Coupon Code Tracking for a potential Customer takes place silently behind-the-scenes via PHP, using an HTTP connection. This method is the most reliable. This method requires an affiliate tracking system like <a href="https://s2member.com/r/s2member-idevaffiliate-commission-tracking/" target="_blank" rel="external">iDevAffiliate</a>, that can track by IP address only, when it needs to. If you\'re running iDevAffiliate, please see the example above.</li>' . "\n";
